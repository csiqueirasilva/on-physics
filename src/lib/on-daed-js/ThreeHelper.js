...
(function() {
	...
	function onMouse( event ) {
		...
		if(ThreeHelper.Intersect.Enabled) {
		
			// update the picking ray with the camera and mouse position	
			raycaster.setFromCamera( mouse, ThreeHelper.Intersect.Camera );	
...
(function () {
	...
	ThreeHelper.Billboard.Create = function (sizeX, sizeY, material) {
		var geo = new THREE.PlaneGeometry(sizeX, sizeY);
		var mesh = new THREE.Mesh(geo, material);
		ThreeHelper.Billboard.Objects.push(mesh);
		return mesh;
	};
	
	ThreeHelper.Billboard.Update = function (camera) {
		for(var i = 0; i < ThreeHelper.Billboard.Objects.length; i++) {
			ThreeHelper.Billboard.Objects[i].rotation.setFromRotationMatrix( camera.matrix );
		}
	};
