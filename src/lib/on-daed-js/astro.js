(function () {
    ...
    ON_DAED["ASTRO"].MIN_JD = 0;
    ON_DAED["ASTRO"].MAX_JD = 1192331101130000255.99999999999999999999999;

    ON_DAED["ASTRO"].getRawDateFromJulian = function (julian) {
    ...
    ON_DAED["ASTRO"].getDateFromJulian = function (julian) {
        ...
        var ret = this.getRawDateFromJulian(julian);
    ...
    ON_DAED["ASTRO"].getNextMoonPhasesFromJulian = function (julian) {
        var newMoon = ON_DAED.ASTRO.getNextMoonPhaseFromJulian(julian, ON_DAED.ASTRO.MoonPhases.NEW);
        var firstQuarterVar = ON_DAED.ASTRO.MoonPhases.FIRST_QUARTER;
        var fullVar = ON_DAED.ASTRO.MoonPhases.FULL;
        var lastQuarterVar = ON_DAED.ASTRO.MoonPhases.LAST_QUARTER;

        var julianInt = parseInt(julian);
        var julianOriginal = julian;

        while (newMoon < julianOriginal) {
            julian = parseFloat(julian) + 30;
            newMoon = ON_DAED.ASTRO.getNextMoonPhaseFromJulian(julian, ON_DAED.ASTRO.MoonPhases.NEW);
        }

        var diff = parseInt(Math.abs(parseInt(newMoon) - julianInt) / 7);

        if (diff > 0) {
            lastQuarterVar = -(1 - ON_DAED.ASTRO.MoonPhases.LAST_QUARTER);
            diff--;
        }

        if (diff > 0) {
            fullVar = -(1 - ON_DAED.ASTRO.MoonPhases.FULL);
            diff--;
        }

        if (diff > 0) {
            firstQuarterVar = -(1 - ON_DAED.ASTRO.MoonPhases.FIRST_QUARTER);
        }

        var phases = {
            new: newMoon,
            firstQuarter: ON_DAED.ASTRO.getNextMoonPhaseFromJulian(julian, firstQuarterVar),
            full: ON_DAED.ASTRO.getNextMoonPhaseFromJulian(julian, fullVar),
            lastQuarter: ON_DAED.ASTRO.getNextMoonPhaseFromJulian(julian, lastQuarterVar)
        };
        
        return phases;
    };

    ON_DAED["ASTRO"].getMoonPhaseFromJulian = function (julian) {
        var phases = ON_DAED["ASTRO"].getNextMoonPhasesFromJulian(julian);
        
        var newDate = phases.new;
        var firstQuarterDate = phases.firstQuarter;
        var fullDate = phases.full;
        var lastQuarterDate = phases.lastQuarter;

        var phase = Math.max(newDate, firstQuarterDate, fullDate, lastQuarterDate);
        var ret;

        switch (phase) {
            case firstQuarterDate:
                ret = this.MoonPhases.FIRST_QUARTER;
                break;
            case fullDate:
                ret = this.MoonPhases.FULL;
                break;
            case lastQuarterDate:
                ret = this.MoonPhases.LAST_QUARTER;
                break;
            case newDate:
                ret = this.MoonPhases.NEW;
        }
